...
const Creators = () => {
  ...
  function fetchData(
    ...
    ? `${BASE_URL_CREATORS}?firstNameStartsWith=${search}&${getAuthQueryString()}&offset=${offset}`
    : `${BASE_URL_CREATORS}?${getAuthQueryString()}&offset=${offset}`;
                    ...
                    creator.comics.items.map((comic: any, index: number) => (
                      <span key={index}>
                        {typeof comic === "string"
                          ? comic
                          : comic && comic.name}{" "}
                        {index < creator.comics.items.length - 1 && ", "}
                      </span>
                    ))
                  ) : (
                    <span>
                      Não foram encontrados Comics desse Criador na base de
                      dados.
                    </span>
                  )}
                </p>
